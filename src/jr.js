// todo mozna ke kazdy zastavce pridat minutaz jak daleko je od zacatku

// todo presnost v casech jizd

export const jizdni_rad = {

	"Štrossova": [
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 501,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 526,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 544,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 603,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 628,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 636,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 648,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 715,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 721,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 729,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 748,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 808,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 900,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 937,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1037,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1117,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1150,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1307,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1337,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1356,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1427,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1437,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1443,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1452,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1514,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1527,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1539,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
				{
			typ: "trol",
			cislo_linky: 12,
			cas: 1550,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1610,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1619,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1636,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1655,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1737,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1745,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1808,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1825,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1845,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 1924,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 2020,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 2128,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 2225,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 2252,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
		{
			typ: "trol",
			cislo_linky: 12,
			cas: 2318,
			zastavky: [
				"Štrossova",
				"U Kostelíčka",
				"Krajský úřad",
				"Náměstí Republiky",
				"Masarykovo náměstí",
				"U Marka",
				"Hlavní nádraží"
			],
			delka_jizdy: 10 // minuty
		},
	],

	"K Nemocnici": [
	],

	"Na Okrouhlíku": [
	],

	"Pardubice-Pardubičky": [
		{
			typ: "vlak",
			cas: 449,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 522,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 606,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 715,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 815,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 915,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1115,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1315,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1413,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1515,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1613,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1715,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1813,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 1915,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 2013,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 2123,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 2311,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
		{
			typ: "vlak",
			cas: 2411,
			zastavky: [
				"Pardubice-Pardubičky",
				"Pardubice hl.n."
			],
			delka_jizdy: 3 // minuty
		},
	],


/// přestupní

	"Náměstí Republiky": [
	],

	"Masarykovo náměstí": [
	],


	"Pardubice hl.n.": [
		{
			typ: "vlak",
			cas: 105,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 434,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 505,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 536,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 606,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 638,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 651,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 705,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 722,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 740,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 806,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 835,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 905,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 933,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1006,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1033,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1105,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1133,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1206,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1233,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1305,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1333,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1406,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1434,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1505,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1534,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1606,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1633,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1705,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1733,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1806,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1833,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1905,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 1933,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 2006,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 2033,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 2105,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 2133,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
		{
			typ: "vlak",
			cas: 2250,
			zastavky: [
				"Pardubice hl.n.",
				"Pardubice-Rosice n.L.",
				"Hradec Králové hl.n."
			],
			delka_jizdy: 25
		},
	],

	"Hradec Králové hl.n.": [
		{
			typ: "vlak",
			cas: 504,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 604,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 704,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 804,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 904,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1104,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1204,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1304,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1404,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1437,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1504,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1604,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1704,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1804,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 1904,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 2004,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 2104,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
		{
			typ: "vlak",
			cas: 2240,
			zastavky: [
				"Hradec Králové hl.n.",
				"Všestary"
			],
			delka_jizdy: 8
		},
	],
};

const aliases = {
	"Masarykovo náměstí": ["Sukova", "Třída Míru", "17. listopadu"],
	"Pardubice hl.n.": ["Hlavní nádraží"],
	"Hradec Králové hl.n.": ["Hradec Králové", "Terminál HD"],
}

for (const [key, values] of Object.entries(aliases)) {
	values.forEach(v => {

		jizdni_rad[v] = jizdni_rad[key];

		for(let i = 0; i < jizdni_rad[key].length; i++) {
			let modifikovane_zastavky = jizdni_rad[key][i].zastavky.map(z => (z == key)? v : z);
			jizdni_rad[v][i] = jizdni_rad[key][i];
			jizdni_rad[v][i].zastavky = modifikovane_zastavky
		}
	});
}

console.log(jizdni_rad)
